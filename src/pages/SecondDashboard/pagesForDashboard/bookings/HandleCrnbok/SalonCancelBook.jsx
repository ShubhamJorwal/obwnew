...
function SalonEndCancelBooking() {
  ...
  useEffect(() => {
    const handleBooking = () => {
      ...
      const userBookingData = JSON.parse(localStorage.getItem("NewBookingData"));

      if (!userBookingData || !userBookingData.id) {
        setErrorMessage("Error: Invalid user booking data.");
        return;
      }

      const { id } = userBookingData;
      
  
      const existingData = localStorage.getItem("BookSelectedData");
      let allSubServices = [];
  ...
  return (
    <div id="createuseendbook">
      {errorMessage ? (
        <div id="concreabook">
          <p>{errorMessage}</p>
          <button style={{margin:"1rem 0"}} id="firstbtn" onClick={handleGoHome}>Go Home</button>
        </div>
      ) : (
        <div id="concreabook">
          <svg
...
export default SalonEndCancelBooking;
